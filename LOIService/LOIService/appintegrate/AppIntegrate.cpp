...
BOOL CAppIntegrate::GetAppDataBlob(CString sSessionName,CString sPeername,CString &sNewPortString,PBYTE *ppByData,DWORD &dwInOutLen)
{
				...
				case IDS_RA_SESSION_NAME:
					{
						...
						bRet= m_pRemoteAsst->CreateInvitation(sPeername, sInvitationFileData,sPort);
						if(bRet)
						{
							sNewPortString+=_T(",S")+sPort;
...
BOOL CAppIntegrate::SetAppDataBlobData(CString sSessionName,BYTE *pByData,DWORD dwInLen)
{
/*	for(int iSessionIndex=0;iSessionIndex<sizeof(MEETINGNAMES)/sizeof(MEETINGNAMES[0]);iSessionIndex++)
	{
		CString  sPreDefinedName;
