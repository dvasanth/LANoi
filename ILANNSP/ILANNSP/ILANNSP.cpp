...
#include	<shlwapi.h>
#include     "dns_sd.h"
#include	"LOIsession//nsp/NSPLOISession.h"
#include   <strsafe.h>
...
DEBUG_LOCAL wchar_t				*	gNSPName			= L"LAN On Internet Name Resolution";
DEBUG_LOCAL GUID					gNSPGUID			= { 0x7e8d5486, 0xabae, 0x4aed, { 0xa1, 0x95, 0xcf, 0xab, 0xd1, 0xf, 0x4e, 0xcc } };
		...
		LPHANDLE				outLookup )
{
	...
	StringCbCopy(szAppName,sizeof(szAppName),_tcsrchr(szAppPath,'\\')+1);
	_tcslwr_s(szAppName,sizeof(szAppName)/sizeof(szAppName[0]));

	require_action( CNSPLOISession::IsLOIEnabled(szAppName), exit, err = WSASERVICE_NOT_FOUND );
...
DEBUG_LOCAL OSStatus	QueryCreate( const WSAQUERYSETW *inQuerySet, DWORD inQuerySetFlags, QueryRef *outRef )
{
	...
	*p = obj;
	...
	char *				dst;
	CHAR				szName[1024]={0};
	StringCbPrintfA(szName,sizeof(szName),"%S",inQuerySet->lpszServiceInstanceName);
	if( obj->name[ 0 ] == '\0' )
	{
		src = szName;
		dst = obj->name;
	...
	StringCbPrintfW(wszName,sizeof(wszName),L"%hs",obj->name );
	if(NSPSession.GetPeerIPv4Address(wszName,*(DWORD*)&obj->addr4))
	{
		obj->addr4Valid = true;
		obj->numValidAddrs++;
	}
