...
void CTrayIconHooker::StartUsingAnimation(HICON* phIcons, int nNumIcons, DWORD dwDelay)
{
  ...
  m_hQueueTimer=CreateTimerQueue();
  CreateTimerQueueTimer(&m_hTimerQueueTimer, m_hQueueTimer,(WAITORTIMERCALLBACK)FrameCallBack, this, 0, (dwDelay),0);
  m_nTimerID=(UINT_PTR)m_hTimerQueueTimer;


}
VOID CTrayIconHooker::FrameCallBack(PVOID pParam, BOOL timerOrWait)
{
	CTrayIconHooker  *pThis=(CTrayIconHooker*)pParam;
	pThis->OnTimer(pThis->m_nTimerID);
}
  ...
  if (m_nTimerID)
  {
   // if (::IsWindow(m_hWnd))
     // KillTimer(m_nTimerID);
	  if (m_hTimerQueueTimer) DeleteTimerQueueTimer(m_hQueueTimer,m_hTimerQueueTimer,INVALID_HANDLE_VALUE);
	  if (m_hQueueTimer) DeleteTimerQueue(m_hQueueTimer);
...
BOOL CTrayNotifyIcon::SetGrayIcon(UINT nIDResource)
{

	HICON  hIcon,hGrayIcon;
	hIcon=LoadIconResource(nIDResource);
	hGrayIcon=CreateGrayscaleIcon( hIcon);
	SetIcon(hGrayIcon);
	DestroyIcon(hIcon);
...
BOOL CTrayNotifyIcon::SetGrayIcon(HICON hIcon)
{
