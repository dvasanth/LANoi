...
void CFreeMenu::DrawMenu()
{
	...
	if(flags==1)
	{
		...
		if(!m_IsVisible)
		{
			m_selfWnd->Create(AfxRegisterWndClass(CS_CLASSDC),L"",WS_EX_TOOLWINDOW,boardRect,m_ParentWnd,1888);
			//m_selfWnd->BringWindowToTop();
			m_selfWnd->UpdateWindow();
//			m_selfWnd->InvalidateRect(boardRect);
			m_IsVisible = TRUE;
	...
	case WM_LBUTTONDOWN:
		{
			...
			if(selected!=-1)
			{
				...
				{    
					...
					{
						UINT iMenuID;
						iMenuID=m_Menu-> GetMenuItemID(selected);
						m_ParentWnd->PostMessage(WM_COMMAND,iMenuID,0);
						//m_selfWnd->BringWindowToTop();
						ClearParent();
						m_selected = -1;
						m_goby = -1;
