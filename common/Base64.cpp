...
LPBYTE CBase64::DecodedMessage() const 
{ 
	return (LPBYTE) m_pDBuffer;
}

CString CBase64::EncodedMessage() const
{ 
	CString			sMessage;

	sMessage.Format(_T("%hs"),(LPCSTR) m_pEBuffer);
	return  sMessage;
}
...
void CBase64::Decode(CString sEncoded,DWORD &dwDecodedLen)
{
	PBYTE		pBuffer=NULL;
	CStringA	sStringA;
	DWORD		dwBufLen;

	//convert unicode to char to convert it to bytes back
	sStringA.Format("%S",(LPCTSTR)sEncoded); 

	pBuffer=(LPBYTE)(LPCSTR)sStringA;
	dwBufLen=sStringA.GetLength(); 
	...
	while((nIndex + 4) <= m_nEDataLen)
	{
		...
		if(m_pEBuffer[nIndex + 3]=='=')
		{
			...
			m_nDDataLen= m_pEBuffer[nIndex + 2]=='='?m_nDDataLen+1:m_nDDataLen+2;
		}
		else
		{
			m_nDDataLen+=3;
